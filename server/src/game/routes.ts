...
gameRouter.get('/data', (_req, res) => {
  res.json({
    ...
    maxDefenseModuleSlots: MAX_DEFENSE_MODULE_SLOTS,
    ...
    boosters: BOOSTERS,
    boosterDurationOptions: BOOSTER_DURATION_OPTIONS,
    kampfBoostMultiplier: KAMPF_BOOST_MULTIPLIER,
    vouchers: SHOP_VOUCHERS,
    containerTypes: CONTAINER_TYPES,
    containerJackpotChance: JACKPOT_CHANCE,
    containerJackpotRewards: JACKPOT_REWARDS,
    ...
    shieldRegenBaseByClass: SHIELD_REGEN_BASE_BY_CLASS,
    shieldRegenDefaultBase: SHIELD_REGEN_DEFAULT_BASE,
    evasionBase: EVASION_BASE,
    evasionMax: EVASION_MAX,
    critChanceBase: CRIT_CHANCE_BASE,
    critChanceMax: CRIT_CHANCE_MAX,
    critDamageMultiplierByClass: CRIT_DAMAGE_MULTIPLIER_BY_CLASS,
    critDamageDefaultMultiplier: CRIT_DAMAGE_DEFAULT_MULTIPLIER,
...
gameRouter.post('/build/defensemodule', (req: AuthedRequest, res) => {
  const { moduleId } = req.body ?? {};
  if (typeof moduleId !== 'string') return res.status(400).json({ error: 'moduleId erforderlich.' });
  handleAction(req, res, (state) => startDefenseModuleUpgrade(state, moduleId));
});
...
gameRouter.post('/shop/booster', (req: AuthedRequest, res) => {
  const { boosterId, durationHours } = req.body ?? {};
  if (typeof boosterId !== 'string') return res.status(400).json({ error: 'boosterId erforderlich.' });
  if (durationHours !== undefined && typeof durationHours !== 'number') {
    return res.status(400).json({ error: 'durationHours muss eine Zahl sein.' });
  }
  handleAction(req, res, (state) => buyBooster(state, boosterId, durationHours));
});
...
// Nutzerentscheidung (Juli 2026, throttled wiedereingefuehrt 30.07.2026 zusammen mit den KI-Bots
// selbst, siehe README): reines 2-Spieler-Koop-Spiel unter vertrauten Mitspielern, keine
// PvP-Konkurrenz - spricht nichts dagegen, den vollen Zustand von KI-Vega/KI-Nyx und den aktiven
// Piratenbasen einsehbar zu machen, um deren Wirtschafts-/Angriffsverhalten zu beobachten. Bewusst
// NUR lesend, kein handleAction (kein tick/save noetig).
