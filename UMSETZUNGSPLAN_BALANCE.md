...
>
> **ENTSCHIEDEN AM 28.08.2026 (siebte Session, Nutzer hat delegiert): ES BLEIBT BEI 0,16 UND
> 0,875, DIE PRAEMIE WIRD AUSGESCHLOSSEN, DIE ZEITPUNKT-ABHAENGIGKEIT ENTFAELLT BEWUSST.**
> Begruendungen in `balance/session2-simulation/reicherfund_13_entscheidungen.txt`, umkehrbar.
> Kurz: der gemessene Unterschied klingt mit der Laufzeit ab (+26,0 % ueber 7 Tage, +14,6 % ueber
> 14, +1,7 % im Erwartungswert) - ueber die Zeitraeume, in denen hier gerechnet wird, ist das
> Niveau unveraendert. Eine Median-Kalibrierung (0,709) waere dagegen eine echte Kuerzung des
> Erwartungswerts um 19 % gewesen und haette genau die Bewegung ausgeloest, die der Absatz
> "DAS NIVEAU BLEIBT" unten ausschliesst.
>
> **ZU BEACHTEN BLEIBT: "Niveau unveraendert" gilt fuer den MITTELWERT, nicht fuer die typische
> Woche.** Der Median steigt um 23,5 %, die gemessene
...
>
> **NACHTRAG 26.08.2026, 30-TAGE-LAUF ALS POSITIVPROBE (ein Lauf, ein Profil - Hinweise, keine
> Belege): DREI KRITERIEN SIND ERSTMALS SICHTBAR VERLETZT, K5 IST IN DIESEM LAUF ERFUELLT.**
>
> | | Wert | |
> |---|---|---|
> | K1 / K1b | 92,0 % / **99,9 %** | **VERLETZT** - Tag 20, 23, 27 je elf Verlustereignisse, Flottenmacht auf 0,00-0,05 Mrd |
> | K4 | Wochen **1-4** ohne Freischaltung | **VERLETZT** - jetzt an den echten Sperren gemessen |
> | K6 | Plateau **26 Tage** ab Tag 4 | **VERLETZT** (Kriterium: keines ueber 5 Tage) |
> | K5 | groesste Quelle 39,8 % | **erfuellt** - siehe Einschraenkung unten |
>
> **Ab Tag 20 vernichtet JEDER Raid die gesamte Heimatflotte**; der Aufbau zwischen zwei
> Checkpoints (rund drei Tage) reicht nicht, um wieder verteidigungsfaehig zu sein. Ohne
> `--treiber=tick` war das unsichtbar.
>
> **EINSCHRAENKUNG ZU K5, WICHTIG:** ueber acht Laeufe liegt der groesste Anteil zwischen **39,8 %
> und 81,9 %** - die 50-%-Schwelle verlaeuft mitten durch die Spanne. Ohne Raid verletzt, mit Raid
> erfuellt, und zwar nicht weil der Reiche Fund kleiner waere, sondern weil der Raid den Nenner
> verbreitert. **K5 ist nicht stabil entschieden**; was entscheidet, ist ein einzelner Wuerfel.
> Deshalb mindestens drei Laeufe je Profil.
>
> **ZWEI KENNZAHLEN WURDEN DABEI REPARIERT** (Nutzer hat die Entscheidung ueberlassen, beide
> umkehrbar):
> - **K1b neu, K1 unveraendert** - Muster K3/K3b. K1 zaehlt je STUNDE, ein Raid laeuft ueber zwoelf
>   Wellen; es unterschaetzt dadurch systematisch (92,0 gegen 99,9 %) und koennte die Schwelle bei
>   gleichmaessigerer Verteilung ganz verfehlen. K1b misst den groessten Rueckgang vom Hoch der
>   letzten 24 Stunden; das Fenster ist begrenzt, sonst zaehlte eine Zermuerbung ueber Wochen als
>   ein Ereignis. Gegenprobe: ohne mehrstuendiges Ereignis liefern beide denselben Wert.
> - **K4 haengt jetzt an den echten Sperren** statt an `npcFloor` (300.000-3.000.000 gegen rund
>   60.000.000 Startflottenmacht - vorher wurden am Tag 0 sieben Sektoren gleichzeitig gemeldet).
>   **Sektoren taugen strukturell nicht:** es gibt keine Sperre, und die Piraten-Sektoren skalieren
>   mit der eigenen Macht mit. Auch Schiffe, Verteidigung und Forschung haben keine Voraussetzung.
>   Gestaffelt ist genau viererlei: Heimatstufe V2/V3 (`HOME_TIER_UNLOCK_LEVELS`, Minen 36/32/30),
>   Stations-Stufe, Imperator (Teile-Sperre), Sandronator (`unique`).
>
...
> Nicht geloescht, sondern nie hochgeladen - `git log --all --diff-filter=A` kennt beide Namen
> nicht, `sim13_geruest.txt` aus derselben Session dagegen schon.
>
> **Damit ist der Ankerwert -2,0 % des Simulationsbuilds nicht mehr reproduzierbar** und taugt
> nicht als Vergleichsanker. Er bleibt als Protokollwert stehen. Betroffen sind alle vier Punkte
> aus `sim13_geruest.txt` Abschnitt 8, nicht nur der erste: Punkt 2 und 3 arbeiten an
> `sim13_lauf.mjs`, Punkt 4 faehrt es.
>
> **GEPRUEFT UND GUELTIG:** `npm install` + `npx tsc -p tsconfig.json` laufen sauber,
> `make_messbuild_kum.mjs /tmp/mb_kum --rf=4 --evk=0.20 --evm=0.08` baut, und
> `MESSBUILD=/tmp/mb_kum node check_build_anker.mjs 40` liefert **normiert -1,8 %** (roh +5,3 %).
> Vier Messungen desselben Ankers liegen jetzt bei -1,1 / -1,8 / -2,3 / -2,8 % - die Streuung ist
> mit rund 2 Prozentpunkten bestaetigt, die roh/normiert-Falle zum vierten Mal ebenfalls.
>
> **AM CODE NACHGEZAEHLT (Anker aus `/tmp/mb_kum`, nicht aus der TS-Quelle):** `missions.js` und
> `groupOps.js` importieren `fleetSizeRewardMultiplier` weiterhin und rufen sie in
> `runHourlyCheck()` (Z. 321) bzw. `runGroupHourlyCheck()` (Z. 713); `game/loot.js` liegt im Build
> und wird von keiner der beiden Dateien referenziert. Was die Verdrahtung braucht, ist an der
> Stelle vorhanden: `npcLosses` (vernichtete Gegner je Typ) und `losses` (eigene Verluste je Typ)
> stehen in `runHourlyCheck()` beide bereits als lokale Objekte zur Verfuegung.
>
> **METHODISCHER VORBEHALT ZUR WIEDERHERSTELLUNG:** wer die neun Block-A-Patches neu schreibt und
> gegen `check_build_anker.mjs` prueft, prueft seine eigene Nachbildung gegen die Vorlage, nach der
> er sie geschrieben hat. Das ist als Machbarkeitsnachweis und als Quelle der Patch-Anker fuer die
> Bauanleitung wertvoll, ersetzt aber nicht die urspruengliche Aussage.
...
> in `runHourlyCheck()` direkt auf den Zaehler multipliziert, und `finalizeMission()` zahlt daraus
> **beides** aus - Container (`combatWins * winContainer.count`) und Ressourcen
> (`winResources * combatWins`).
>
> Die Bauanleitung nimmt dem Zaehler beide Aufgaben ab: der Container-Fund faellt kuenftig EINMAL
> je Mission, `winResources` haengt an der Kurve. **Damit haben die zwei Verdoppler keinen Ort
> mehr, und die Bauanleitung sagt nicht, wo sie hin sollen.** Die Referenzschleife in
> `check_build_anker.mjs` kennt keinen von beiden - Probe und Referenz koennen also nur
> uebereinstimmen, wenn das vorher geklaert ist.
>
> Drei Wege, keiner entschieden: (1) beide Verdoppler auf den Kurvenfaktor der Ressourcen legen
> (dann wirken sie weiter, aber nur noch auf den Ressourcenteil); (2) beide auf den einen
> Container-Fund je Mission legen (dann bleiben sie am Container, verlieren aber die Kopplung an
> die Zahl der Siege); (3) ersatzlos streichen und den Sandronator-Vorteil ueber eine andere
> Groesse fuehren. **Wirkung ungemessen.** Der Plan warnt weiter unten bereits, dass sich jede
> neue Skalierung auf `PIRATEN_EVENT_BONUS_MULTIPLIER` draufmultipliziert - genau dieser Fall.
...
>
> **OFFEN, NUTZERENTSCHEIDUNG: der Wert von f.** *Nachtrag 21.08.2026, nachdem der Nutzer die
> Entscheidung delegiert hat: EINGETRAGEN IST f = 12, als Empfehlung und jederzeit umkehrbar -
> nichts davon ist gebaut.* Begruendung: 13.1-A (Decke) kippt bei f = 13,5, f = 12 nutzt sie mit
> 8 % Sicherheitsabstand aus; und seit dem Ausgang von Entscheidung 17 entscheidet f im
> Wesentlichen ueber die KI-MITSPIELER statt ueber die Piratenbasen (bei denen `garrisonReadiness()`
> bei 1,0 gedeckelt ist, mehr Einkommen also nur die Farm-Erholung beschleunigt). Bot-Flotten
> tauchen dagegen im Elite-Bollwerk und in der Raid-Verstaerkung tatsaechlich auf - das spricht fuer
> den groesseren der beiden Werte. Wer das anders sieht, setzt f = 8 (0,67 / 0,19 / 0,26).
> Vorschlag **f = 12** (ausgereizt, nutzt die Decke bis auf 8 %) wenn "begleiten" und die
> Startphase (13.5) den Ausschlag geben; **f = 8**
...
### Entscheidung 17 - Piratenbasen als Bedrohung: VERWORFEN am 21.08.2026

> **AUSGANG (Nutzerentscheidung 21.08.2026): DIE OFFENSIVE DER PIRATENBASEN WIRD ABGESCHALTET.
> Basen und KI-Mitspieler BLEIBEN, die Bedrohung wandert vollstaendig auf den RAID.**
> Der Messkasten unten bleibt als Begruendung stehen - er dokumentiert, warum der Weg nicht
> gangbar ist, und mehrere seiner Befunde gelten fuer den Raid weiter.
>
> **Warum verworfen:** die Messung hat die Grundannahme widerlegt. Damit eine Basis mit ihrem
> ECHTEN Bestand wehtut, braeuchte sie rund das Zehn- bis Fuenfzehnfache der Spielerflotte,
> dauerhaft - das waere nicht "manchmal eine Bedrohung", sondern permanent uneinholbar und das
> Gegenteil des Ziels aus Entscheidung 13.1. Der Ausweg (Welle am Ziel skalieren) liefert dagegen
> keinen echten Gegner, sondern eine gestellte Begegnung - **und damit exakt dasselbe, was der Raid
> ohnehin tut** (`processRaidWave()` erzeugt die Angreifer ueber `generateFallbackFleet()` aus dem
> Nichts, skaliert auf die kombinierte Macht des Spielers, zwoelfmal). Zwei Systeme fuer dieselbe
> Abstraktion, wovon nur eines bereits richtig funktioniert.
>
> **Was ABGESCHALTET wird:** `runPirateBaseOffensiveTurn()` samt der 1,15-Marge. Der Zweig loest
> heute faktisch nie aus, kostet also nichts. Beim Abschalten ist der Ruecklauf noch fliegender
> `PirateBaseOffensiveDeployment`-Eintraege zu bedenken (nach dem Reset gibt es keine, im
> laufenden Betrieb schon).
>
> **Was BLEIBT:** die Piratenbasen als Farm-Inhalt (gemessen 1,60 Mrd netto je Angriff, mit vier
> Basen und 20 h Erholung rund 5,9-6,4 Mrd/Tag = 8-10 % der Tageseinnahmen), Entscheidung 5
> unveraendert, die KI-Mitspieler und damit Entscheidung 13 samt der Kalibrierung vom 21.08.2026.
>
> **FOLGE FUER DIE NOCH OFFENE ZAHL f AUS 13.1, ausdruecklich:** mit abgeschalteter Offensive hat
> der Flottenbestand einer Basis nur noch EINE Wirkung - `garrisonReadiness()` ist bei 1,0
> gedeckelt, Wachstum zaehlt also allein fuer die ERHOLUNG nach dem Leerfarmen. Alles darueber ist
> totes Gewicht. **Mehr NPC-Einkommen beschleunigt bei den Basen nur die Farm-Erholung und sonst
> nichts.** Die Zahl f entscheidet damit im Wesentlichen ueber die KI-MITSPIELER, deren Flotten im
> Elite-Bollwerk und in der Raid-Verstaerkung tatsaechlich auftauchen. Das war bei der Kalibrierung
> am Vormittag noch nicht so.
>
> **UEBERTRAGBAR AUF DEN RAID** (die Bedrohung liegt jetzt dort):
> - Der Spieler steht im Raid bereits auf Seite A - Forschung, Kuppel-Pool, `homeDefense` und
>   Rueckzug wirken dort korrekt. Der Seitentausch-Befund ist damit erledigt, nicht verloren.
> - **Das Kliff gilt auch fuer den Raid.** `raid.txt`: an jedem entwickelten Stand 12/12 Wellen und
>   100 % perfekte Abwehr, im Fall "schwach" 10,9 Wellen und 94,4 % Flottenverlust. Zwischen
>   23,8 % und 94,4 % liegt nichts. Wer den Raid haerter dreht, laeuft in dieselbe Wand.
> - Der Rueckzug rettet Schiffe und opfert Anlagen (-3,0 Punkte Flotte, +4,8 Punkte Wert) - im
>   Raid steht er auf `'fleetOnly'` und ist damit eine bereits getroffene Entscheidung.
> - `homeDefense` ist kein Verteidiger-Vorteil, sondern der Klassen-Schalter: fuer einen KANONIER
>   +7,3 Punkte Verlust, fuer ein Bollwerk umgekehrt. Fuer Bollwerk NICHT gemessen.
...
**Offene Teilpunkte - ALLE HINFAELLIG mit dem Ausgang oben.** 17.1 bis 17.5 werden nicht weiter
verfolgt. Was von ihnen ueberlebt, wandert nach Entscheidung 18:
- **17.3 (Haeufigkeit)** ist im Raid bereits geloest - Raids sind ohnehin je Spieler getaktet.
- **17.4 (Neulingsschutz)** ist im Raid bereits vorhanden (`isNewcomerProtected()` steht dort).
  Der Befund, dass er AUSSERHALB von `raids.ts` nirgends greift, bleibt als Falle vermerkt.
- **17.5 (Beute)** entfaellt mit der Offensive.
- Die Zahl der Basen bleibt unveraendert bei vier. Sie ist zuerst ein EINNAHMEN-Regler (bei zwoelf
  Basen rechnerisch 18-19 Mrd/Tag statt 5,9-6,4) und war nie das Mittel gegen fehlende Bedrohung.
...
> `WAVES`/`ONLY`/`FIXPOWER` erweitert - ohne diese Variablen unveraendert). Alle Werte aus dem
> kumulativen Messbuild. **`RAID_WAVE_ROLL` wurde NICHT angefasst.**
>
> **`raid.txt` IST FUER DIE KALIBRIERUNG NICHT MEHR GUELTIG** - es stammt aus der Zeit vor
> Entscheidung 16. Neue Baseline (40 Raids je Fall, 12 Wellen, Reparatur 0,70):
>
> | Fall | Wellen | perfekt | Flottenverlust | Verteidigungsverlust |
> |---|---|---|---|---|
> | voll / grosse Flotte | 12,0 | 100 % | 20,0 % | **0,0 %** |
> | voll ohne Kampf-Boost | 12,0 | 100 % | 36,4 % | **0,0 %** |
> | mittel / grosse Flotte | 12,0 | 100 % | 28,7 % | **0,0 %** |
> | voll / kleine Flotte | 11,5 | 57 % | 23,7 % | 26,9 % |
> | schwach / kleine Flotte | 11,0 | 3 % | 94,0 % | 60,4 % |
>
> **BEFUND A - ENTSCHEIDUNG 16 MACHT VERTEIDIGUNGSANLAGEN IM RAID PRAKTISCH UNZERSTOERBAR.**
> Direkte Gegenprobe Repo-Stand gegen Messbuild, identische Faelle: Stand mittel
> **21,6 % -> 0,0 %** Verteidigungsverlust; Stand "voll / kleine Flotte" **92,0 % -> 35,4 %**,
> waehrend dort der Flottenverlust von 14,5 auf 39,0 % steigt - die Verteidigung haelt, die Flotte
> zahlt. Deutung: unter Klassen-RapidFire haben die generierten NPC-Wellen
> (`generateFallbackFleet()`, reine Schiffsflotten) keine RapidFire-Beziehung mehr zu den
> Verteidigungsklassen, die alte Konter-Leiter hatte eine. **Verteidigung wird dadurch im Raid
> faktisch kostenlos: einmal gebaut, nie wieder verloren.** Das beruehrt auch die Kopplung der
> Verteidigungswerte an die Kosteneffizienz der Schiffe - wenn Verteidigung nichts mehr kostet,
> stimmt die Relation nicht mehr. **Gehoert zu ENTSCHEIDUNG 16 und ist dort zu entscheiden, bevor
> sie gebaut wird. Hier nicht weiter verfolgt.**
>
> **BEFUND B - DIE REPARATURQUOTE IST ALS REGLER TOT.** Stand mittel: 0,70 -> 24,3 %,
> 0,30 -> 26,2 %, 0,00 -> 28,4 % Flottenverlust, ueberall 12/12 und 100 % perfekt. Grund ist
> Befund A - es gibt nichts zu reparieren. **Der vor der Messung als erster Kandidat
> vorgeschlagene Regler ist damit widerlegt**, ausdruecklich protokolliert.
>
> **BEFUND C - WELLENZAHL UND GEWICHTE SIND GRADUELLE KOSTENREGLER, OHNE KLIFF** (40 Raids):
>
> | Wellen | voll | mittel | schwach |
> |---|---|---|---|
> | 12 | 20,0 % | 28,7 % | 94,0 % (3 % perfekt) |
> | 18 | 26,3 % | 37,7 % | 96,4 % (3 % perfekt) |
> | 24 | 35,8 % | 45,8 % | 97,9 % (3 % perfekt) |
>
> Verteidigungs-Gewicht (mittel, 10 Raids): 0,3 -> 32,8 %, 1,0 -> 33,1 %, 2,0 -> 42,9 %,
> 4,0 -> 59,8 %. Beide glatt, kein Sprung. **Die KOSTEN eines Raids sind frei einstellbar, und der
> Regler trifft die entwickelten Konten, nicht die schwachen.**
>
> **BEFUND D - DIE VERLIERBARKEIT IST MIT DEN OFFENEN REGLERN STRUKTURELL NICHT ERREICHBAR.**
> In JEDER gemessenen Zelle gewinnen die entwickelten Staende 100 % der Raids perfekt - bei 12,
> 18, 24 und 36 Wellen, bei Reparatur 0,00 und bei Verteidigungs-Gewicht 4,0. **Ursache:**
> `processRaidWave()` berechnet `combinedPower` fuer JEDE Welle neu aus der AKTUELLEN, bereits
> dezimierten Flotte. Schrumpft die Flotte, schrumpft der Gegner mit - eine negative
> Rueckkopplung, die ein Zurueckfallen ausschliesst.
> *Gegengeprueft, ob das Einfrieren der Bemessungsgrundlage reicht* (`raid.initialCombinedPower`
> liegt in `raids.ts` bereits vor, bisher nur fuer den Belohnungsbonus): **nein.** Es trifft
> ausschliesslich die Schwachen (schwach 10,9 -> 3,5 gewonnene Wellen, Totalverlust) und laesst
> voll und mittel unveraendert bei 100 % perfekt.
> **Ueber Sieg oder Niederlage entscheidet allein die Staerke EINER Welle - also `RAID_WAVE_ROLL`,
> und der ist gesperrt. Mit den offenen Reglern ist "Raid haerter" erreichbar, "Raid verlierbar"
> nicht.** Das ist ein Messergebnis, kein Scheitern - die Zelle kann die Frage nicht entscheiden,
> und das war vorher nicht bekannt.
>
> **EMPFEHLUNG (nicht gebaut): `RAID_WAVE_COUNT` von 12 auf 18.** voll 20,0 -> 26,3 %,
> mittel 28,7 -> 37,7 %, schwach praktisch unveraendert. Einziger Regler ohne neue Konstante,
> glatt, kein Kliff. 24 Wellen waeren tragbar, gehen bei Vollausbau aber Richtung "jeder zweite
> Raid halbiert die Flotte". **Beruehrt Entscheidung 3** - mehr Wellen heisst auch mehr Container,
> das Verhaeltnis von Ertrag zu Risiko ist danach gegenzupruefen.
>
> **NACHTRAG 21.08.2026 - ESKALIERENDE WELLEN UND BUNKERBRECHER (NUTZERVORSCHLAG). Der Ausweg
> aus Befund D ist gefunden, ohne `RAID_WAVE_ROLL` anzufassen.**
> Vorschlag: die Wellen phasenweise staffeln (erste vier mittel, naechste vier schwer, letzte vier
> extrem) und in den spaeten Wellen viele Bomber einsetzen.
> *Zur Abgrenzung gegen die Sperre:* das ist eine NEUE Groesse, keine Aenderung am Wuerfel - der
> bleibt unangetastet und wirkt obendrauf. Inhaltlich tut die Eskalation aber dasselbe, sie ist
> deshalb ausdruecklich als Nutzerentscheidung gefuehrt und nicht als Ausweg um die Sperre herum.
>
> **BEFUND E - DIE ESKALATION LOEST DIE WAND AUS BEFUND D AUF.** Sie ist der einzige gefundene
> Regler, der nicht nur die Kosten, sondern den AUSGANG bewegt - weil sie das Verhaeltnis EINER
> Welle zum Verteidiger hebt, und genau daran haengt Sieg oder Niederlage. Die negative
> Rueckkopplung bleibt als Daempfer bestehen, **es entsteht deshalb kein Kliff wie bei
> Entscheidung 17, sondern ein kontinuierlicher Uebergang.**
>
> **BEFUND F - DIE BUNKERBRECHER-WELLE HOLT DIE VERTEIDIGUNG ZURUECK INS SPIEL** und ist damit die
> direkte Gegenmassnahme zu Befund A. Bei identischer Eskalation: Verteidigungsverlust
> **0,2 % -> 41,2 %** (voll) und **3,8 % -> 38,8 %** (mittel), waehrend der Flottenverlust fast
> gleich bleibt (44,0 -> 43,0 %). Die Bomber VERLAGERN den Schaden, statt zusaetzlichen
> anzurichten. Hintergrund: der Bomber ist der einzige Bunkerbrecher (RapidFire 20/20/10 gegen
> Raketenwerfer/Leichtes Laser/Schweres Laser), aber die Wellenprofile gewichten den Schiffspool
> nur nach POSITION im `SHIPS`-Array und kennen keine Rolle. **Fuer den Einbau waere ein viertes
> Profil `bunkerbrecher` die naheliegende Form.**
>
> **Drei Kandidaten, 40 Raids je Fall, jeweils mit 50 % Bomber-Anteil in der letzten Phase:**
>
> | Fall | Ist | A (1/1,20/1,50) | C (1/1,35/2,20) | B (1/1,50/2,50) |
> |---|---|---|---|---|
> | voll / grosse Flotte | 100 % / 20,0 % | 100 % / 34,3 % | 100 % / 56,1 % | **93 %** / 65,2 % |
> | voll ohne Kampf-Boost | 100 % / 36,4 % | 100 % / 57,2 % | **48 %** / 82,7 % | **38 %** / 87,4 % |
> | mittel / grosse Flotte | 100 % / 28,7 % | 100 % / 43,4 % | **80 %** / 69,2 % | **65 %** / 74,1 % |
> | voll / kleine Flotte | 57 % / 23,7 % | 33 % / 50,1 % | 3 % / 88,0 % | 0 % / 94,8 % |
> | schwach / kleine Flotte | 3 % / 94,0 % | 0 % / 99,6 % | 0 % / 100,0 % | 0 % / 100,0 % |
>
> (Angabe: perfekte Abwehr / mittlerer Flottenverlust. Verteidigungsverlust steigt von 0,0 % auf
> 31-49 % an den grossen Staenden.)
>
> **BEFUND G - DER KAMPF-BOOSTER WIRD ZUM AUSSCHLAGGEBENDEN FAKTOR.** In allen drei Kandidaten
> kippt "voll ohne Kampf-Boost" als erster entwickelter Stand (100 / 48 / 38 % perfekt gegen
> 100 / 100 / 93 % mit Booster). Bei Kandidat C entscheidet der Booster ueber knapp die Haelfte
> aller Raids. **Der Raid wuerde damit zum staerksten Kaufargument fuer den Kampf-Booster im ganzen
> Spiel** - eine Design-Aussage, die vor der Wahl eines Kandidaten zu entscheiden ist.
>
> **BEFUND H - DIE ESKALATION TRIFFT DIE SCHWACHEN AM HAERTESTEN, ALSO IN DIE FALSCHE RICHTUNG.**
> "schwach / kleine Flotte" geht bei JEDER Eskalation auf 100 % Totalverlust. Entscheidung 10 hilft
> nicht, weil ihr Neulingsschutz ZEITBASIERT ist (14 Tage) - ein schwach ausgebautes Konto steht
> danach ungeschuetzt da. **Wer eskalierende Wellen einbaut, braucht dazu eine
> ausbaustandsabhaengige Untergrenze**, sonst ist der Raid fuer jeden, der pausiert hat, ein
> garantierter Totalverlust. Nicht gemessen, zwingend mitzuentscheiden.
>
> **EMPFEHLUNG: Kandidat A (1 / 1,20 / 1,50 mit 50 % Bomber-Anteil) als erster Schritt.** Er
> verdoppelt den Flottenverlust an den entwickelten Staenden, holt die Verteidigung mit rund einem
> Drittel Verlust zurueck und laesst "schwach" praktisch unveraendert. Verlierbar wird der Raid
> damit noch nicht - bewusst, weil Befund G und H vorher zu klaeren sind. Kandidat C ist der
> naechste Schritt, sobald beide entschieden sind.
>
> **OFFEN, NUTZERENTSCHEIDUNG:** (1) Kandidat A, C oder B. (2) Befund G - soll der Kampf-Booster
...
Design-Grundlage statt Messung) - dieser Punkt loest ihn jetzt mit Zahlen ein.

**Ist-Zustand, bereits gemessen** (`raid.txt`, 40 komplette Raids je Fall):

| Fall | Wellen gewonnen | perfekt | Flottenverlust | Verteidigungsverlust |
|---|---|---|---|---|
| voll / grosse Flotte | 12,0 | **100 %** | 12,7 % | 20,8 % |
| voll ohne Kampf-Boost | 12,0 | **100 %** | 26,8 % | 44,8 % |
| mittel / grosse Flotte | 12,0 | **100 %** | 23,8 % | 13,8 % |
| voll / kleine Flotte | 12,0 | **100 %** | 13,2 % | 91,9 % |
| schwach / kleine Flotte | 10,9 | 0 % | **94,4 %** | 69,5 % |

**Ein entwickelter Spieler gewinnt jede Welle in jedem Lauf.** Und der eine Fall, der verliert,
verliert fast alles. **Das ist dasselbe Kliff wie in Entscheidung 17** - die eigentliche Frage
lautet deshalb nicht "wie viel haerter", sondern **an welchem Regler die Haerte graduell reagiert,
statt zu kippen.**

**Gesperrt, solange der Nutzer nichts anderes sagt:** `RAID_WAVE_ROLL` (heute
`[1,20 | 1,70 | 2,30-2,50]`) - der direkteste Regler und ausdruecklich auf der
Nicht-anfassen-Liste. Wird NICHT von sich aus angefasst.

**Verfuegbare Regler:**

| Regler | heute | Wirkform |
|---|---|---|
| `DEFENSE_REPAIR_PERCENT` | 0,70 | **graduell** - Abnutzung kumuliert ueber die Wellen |
| `RAID_WAVE_COUNT` | 12 | **graduell** - mehr Wellen, mehr Abnutzung |
| `RAID_FLEET_POWER_WEIGHT` / `_DEFENSE_` | 0,7 / 0,3 | gleichmaessig, aber je Welle |
| `retreatMode` | `'fleetOnly'` | sprunghaft - Kliff-Gefahr |
| Wellenprofil, `battleModifier` | - | sprunghaft |

**Erster Kandidat ist die Reparaturquote.** Sie greift nicht in einen einzelnen Kampf ein, sondern
laesst die Verteidigung ueber zwoelf Wellen abnutzen - genau die Form, die ein Kliff vermeidet.
Sie hat ausserdem bereits einen Klassen-Gegenspieler (`CLASS_BOLLWERK_DEFENSE_REPAIR_PERCENT` =
0,9 gegen 0,7), der dadurch endlich Gewicht bekaeme.

**Messplan (noch nicht begonnen, wartet auf Freigabe):**
1. **Trennschaerfe zuerst.** Sweep ueber Reparaturquote (0,70 bis 0,30) und Wellenzahl (12 bis 20).
   Frage an den Extremwerten: gibt es ueberhaupt eine Einstellung, bei der ein entwickelter
   Spieler MANCHMAL eine Welle verliert, statt entweder nie oder fast alles?
2. **Kriterium festlegen, bevor kalibriert wird.** Anteil perfekter Abwehren in ein Band, plus eine
   Obergrenze fuer den Flottenverlust in der Verlust-Zelle. Beides vorher auf Trennschaerfe
   geprueft.
3. **Kalibrieren gegen alle fuenf Faelle**, damit "schwach" nicht weiter abstuerzt. Entscheidung 10
   (Neulingsschutz, 14 Tage) bleibt die Untergrenze.

**Beruehrt:** Entscheidung 3 (Raid-Ertrag, Variante 6, entschieden und ungebaut) - wird der Raid
haerter, aendert sich das Verhaeltnis von Ertrag zu Risiko, und Abnahmekriterium 5 haengt daran.
Reihenfolge deshalb: erst die Haerte, dann Entscheidung 3 gegenpruefen.
...
| R16 | **NEU 20.08.2026, Nutzerfund - eine Flotte laesst sich in beliebig viele gleichzeitige Gruppen-Operationen aufteilen.** Nutzermeldung: "Elite Bollwerk kann man unendlich mal starten gleichzeitig." Im Code bestaetigt: `createGroupOperation()` prueft nur Sektor, Schiffstypen und Bestand, `respondToGroupOperation()` erlaubt beliebig viele gleichzeitig angenommene Einladungen, `performGroupOperationStart()` kennt keine Mindestteilnehmerzahl. **Das ist ein Defekt, keine Balance-Frage** - der dieselbe Sperre braucht, die es bei Solo-Missionen seit dem 29.07.2026 gibt (`missions.ts` Zeile 97, "immer nur eine Piraten-Sektor-Stufe gleichzeitig"), und die bei den Gruppen-Operationen nie nachgezogen wurde. **Reparatur: eine aktive Operation je Spieler**, geprueft an BEIDEN Eintrittspunkten - wer bereits in einer Operation mit Status `inviting` oder `departed` steckt, kann weder eine zweite erstellen noch eine weitere Einladung annehmen. Gilt fuer P9 und P10 gemeinsam, unabhaengig davon, wie Entscheidung 4.8 spaeter gebaut wird. Braucht KEINE neue Balance-Zahl. **Nutzerentscheidung 20.08.2026: erst zum Server-Neustart einbauen, nicht vorher** - bis dahin darf der alte Stand ausgespielt werden. **Ebenfalls Nutzerentscheidung: der Solo-Start einer Gruppen-Operation bleibt erlaubt** (beide Multiplayer-Sektoren sollen allein beflogen werden koennen), es geht ausschliesslich um die Gleichzeitigkeit | `game/groupOps.ts` (`createGroupOperation()` ~Zeile 73, `respondToGroupOperation()` ~Zeile 174) | Nutzerfund 20.08.2026 |
...
> **R16 - BEFUNDKASTEN, 20.08.2026. Eine Flotte, beliebig viele gleichzeitige Expeditionen.**
> Anlass: Nutzerbeobachtung beim Spielen ("Elite Bollwerk kann man unendlich mal starten
> gleichzeitig"). Im Code gepruefte Bestaetigung, keine Messung noetig - der Defekt liegt in
> fehlenden Pruefungen, nicht in einer Zahl.
>
> **Was fehlt (drei Stellen, alle in `groupOps.ts`):**
> - `createGroupOperation()` prueft Sektor, erlaubte Schiffstypen und Bestand. Nicht, ob der
>   Spieler bereits eine Operation laufen hat.
> - `respondToGroupOperation()` prueft dasselbe - man kann beliebig viele Einladungen
>   gleichzeitig annehmen.
> - `performGroupOperationStart()` kennt keine Mindestteilnehmerzahl. Ein Spieler kann allein
>   erstellen und sofort starten (ohne Rendezvous-Wartezeit, weil nur der Ersteller beteiligt
>   ist). **Das bleibt so gewollt** - siehe Nutzerentscheidung unten.
>
> **Warum es teuer ist: die Belohnung je gewonnenem Check ist flach und PRO TEILNEHMER,
> unabhaengig von der eingesetzten Flotte** (`sectors.ts`, `piraten_elite`):
> - `winResources` 300/180/120 Mio = **930 Mio Wert-Einheiten**, bewusst NICHT eskaliert
> - `guaranteedContainers` 4x Silber + 3x Gold + 2x Elite = **1.097 Mio Wert**
> - `lootBase` 77,5 Mio Wert, mal 2^Siegserie (`REWARD_ESCALATION` Modus `double`)
>
> Ueber 6 Checks (`PIRATEN_CHECK_COUNT`, 24 h) ergibt das rund **17 Mrd Wert je Expedition und
> Teilnehmer**, und keine dieser Zahlen haengt an der Flottengroesse. Die Gegnerstaerke dagegen
> skaliert proportional mit (`PIRATEN_MULTIPLIER_ROLL` 0,90/1,20/1,55). **Wer seine Flotte auf N
> Operationen aufteilt, kaempft N-mal gegen dieselbe relative Schwierigkeit und kassiert N-mal
> die flache Belohnung.** Der `npcFloor` von 3 Mio Macht setzt nur eine Untergrenze von rund
> 3,4 Mio Flottenwert je Teilflotte - bei einer Flotte von 500 Mio Wert sind das ueber hundert
> moegliche parallele Laeufe.
>
> **Mechanisch ist das derselbe Effekt wie Befund 1 in `novice_bonus.txt`** (flache Belohnung
> plus mitskalierender Gegner laesst die Einnahme mit der Flottengroesse fallen) - dort wirkt er
> als Nachteil fuer grosse Flotten, hier als Multiplikator fuer den, der aufteilt. **Regel fuer
> kuenftige Inhalte:** wo eine flache Belohnung auf einen proportional mitskalierenden Gegner
> trifft, MUSS die Zahl gleichzeitiger Teilnahmen begrenzt sein, sonst ist Aufteilen immer
> besser als Zusammenhalten.
>
> **Was im Plan schon stand und was nicht.** Entscheidung 4.8 sieht einen Cooldown vor, aber nur
> fuer P10 und nur gegen *wiederholtes* Starten hintereinander. `FINALE_BALANCE_CHECKLIST.md`
> (Befund 8) vermerkt "kein Cooldown, kein Mindestteilnehmer" fuer beide Multiplayer-Sektoren.
> **Der GLEICHZEITIGE Fall stand nirgends** - und ein Cooldown allein schliesst ihn auch nicht,
> solange er nicht als Tageskontingent gebaut wird.
>
> **Nutzerentscheidungen vom 20.08.2026:**
> 1. **Reparatur: eine aktive Operation je Spieler**, geprueft an beiden Eintrittspunkten. Keine
>    neue Balance-Zahl, dieselbe Regel wie bei Solo-Missionen.
> 2. **Der Solo-Start bleibt erlaubt.** Beide Multiplayer-Sektoren sollen allein beflogen werden
>    koennen - das ist gewollt und wird NICHT mitrepariert.
> 3. **Einbau erst zum Server-Neustart**, nicht vorher. Bis dahin darf der alte Stand ausgespielt
>    werden. Der Fix geht mit dem Sammel-Einbau live, wie alles andere.
>
> **Nicht Teil dieser Reparatur** und weiterhin offen: die Hoehe der flachen Belohnungen selbst
> (930 Mio `winResources` plus 1.097 Mio garantierte Container je Check sind nach der Sperre
> immer noch die groesste Einzelquelle, die ein Spieler willentlich ausloesen kann) und die
> Kadenz aus Entscheidung 4.8.
...
- **`MAX_PLAYER_SHIPS` erneut entscheiden (NEU 11.08.2026, geschaerft 21.08.2026).** Steht auf
  1.000.000. Bis Entscheidung 2 (Beute-Kurve) gebaut und gemessen ist, wirkt das Limit als
  Ersatz-Bremsklotz gegen Weglauf-Wachstum grosser Flotten - danach ist es wieder eine reine
  CPU-Frage. **Nicht vorher entfernen.** Herleitung in Abschnitt 2a, Punkt 8.
  - **Die CPU-Frage ist am 21.08.2026 im Echtbetrieb beantwortet (Nutzerbeobachtung):** bei rund
    1 Mio. Schiffen samt gleichzeitiger Koop-Expedition ins Elite-Bollwerk langweilen sich beide
    CPUs - eine fuer den Worker, eine fuer den Hauptthread. Damit ist die 26-ms-Messung nicht mehr
    nur simuliert. Der erste der beiden urspruenglichen Gruende (Auffangboden gegen einen
    kuenftigen Pro-Einheit-Pfad) ist damit ein Restrisiko, keine gemessene Grenze; es bleibt
    allein der Bremsklotz-Grund.
  - **Reihenfolge fuer das Entfernen, ausdruecklich NICHT beim Reset selbst:** (1) Reset mit
    Limit - es kostet nichts, weil monatelang niemand in die Naehe kommt. (2) Entscheidung 2 im
    echten Spiel laufen lassen, bis wieder jemand in den Bereich kommt, in dem die Kurve greift.
    (3) Dort messen, ob das Flottenwachstum konvergiert. (4) Erst danach entfernen, und dabei
    R13 mit aufraeumen - die Ratschen-Logik (`state.shipLimitCeiling`) wird ohne Limit toter Code.
  - **Offen und ungemessen:** Entscheidung 2 ist keine reine Bremse. Sie senkt die Steigung
    (Exponent 0,85), hebt aber das Niveau - Solo Hoch geht fuer die reale Flotte von -2,32 auf
    +2,42 Mrd netto. Heute bremst sich grosses Wachstum teilweise selbst, weil sich Fluege ab
    einer gewissen Groesse nicht mehr lohnen; genau das faellt weg. Ob der Exponent allein zu
    einem konvergierenden Wachstum fuehrt, ist nicht gemessen - das ist die Frage aus Punkt (3).
  - **`POOL_SIZE` steht weiterhin auf 1** und koennte laut Nutzer auf 2 gehen. Zwei Hinweise fuer
    den Fall: bei zwei Kernen konkurrieren dann zwei Worker plus Hauptthread um zwei CPUs, der
    Gewinn ist nur bei tatsaechlich gleichzeitigen Kaempfen real; und Entscheidung 13.1 hat den
    Bot-Ertragsweg (b) unter anderem mit `POOL_SIZE = 1` begruendet - das Argument wird schwaecher,
    die Entscheidung haengt aber nicht daran.
...
| Datum | Aenderung |
|---|---|
| 30.08.2026 (Nutzerfrage, nachgerechnet) | **Die Bot-Reserve waechst schneller als das Bot-Einkommen - der Flottenaufbau laeuft strukturell aus.** Nachtrag in `bot_baurate.txt` Abschnitt 4b, nichts gebaut. Gebaeudekosten wachsen mit `costGrowth` x1,55 je Stufe (1,6 bei Kristall/Deuterium), die Produktion nach `levelScaledValue()` nur mit x1,13 - **die Schere oeffnet sich dauerhaft um Faktor 1,37 je Stufe** (Sparzeit naechste Metallmine: Stufe 21 rund 6.400 h, Stufe 33 rund 249.000 h, Stufe 40 rund 2,27 Mio h). **Das trifft die Flotte, weil `spendableResources()` die VOLLEN Kosten der naechsten Gebaeude- und Forschungsstufe abzieht:** liegt das Guthaben darunter, ist `spendable` exakt null und der Bot baut kein einziges Schiff. Da die Mine sofort gekauft wird, sobald sie bezahlbar ist, faellt das Guthaben danach wieder auf nahe null bei 1,55-mal hoeherer Reserve - der Zeitanteil ohne Schiffbau geht gegen 100 %. Die Flotte faellt nicht von selbst, der Nachbau stoppt; nach unten gedrueckt wird sie durch die laufenden Verluste. **Pruefbare Vorhersage, an R16 gekoppelt:** heute unsichtbar, weil die Elite-Beute unter dem Mehrfachflug-Glitch das Mineneinkommen um Groessenordnungen uebersteigt; **nach dem R16-Fix muss der Flottenaufbau binnen Tagen stillstehen und die Stueckzahl sinken** - tritt das nicht ein, ist die Rechnung falsch und gehoert nachgemessen. **Loesungsansatz, nicht entschieden und NACH dem Bauen:** die Reserve auf einen Anteil des Guthabens begrenzen statt auf die vollen Kosten; eine Stelle, messbar mit `run_bot_baurate.mjs` ohne neues Werkzeug. |
| 29.08.2026 (siebte Session, vierter Teil) | **Bot-Baurate gemessen (Nutzerbeobachtung an KI-Nyx/KI-Vega). Die Ausgangsvermutung war falsch. NICHTS GEBAUT.** Protokoll `bot_baurate.txt`, Rohdaten `bot_baurate.json`, neue Werkzeuge `make_messbuild_botrate.mjs` und `run_bot_baurate.mjs`. **Der Stueckzahldeckel in `maybeBuildShips()` ist nicht der Hauptengpass:** er wirkt (Deckel 500 statt 5 gibt x2,76 Kampfschiffe), aber das Restguthaben bleibt in allen fuenf Zellen bei 12,4-12,6 Mrd. **Ursache ist `spendableResources()`**, das die Kosten des naechsten Gebaeudes UND der naechsten Forschung reserviert, bevor ein Schiff erwogen wird - bei Minenstufe 21 zweistellig in Milliarden. Von 31,3 Mrd Ausgaben in 7 Tagen landen nur 3,2-4,1 Mrd in Flotte und Verteidigung. **Die dynamische Kopplung an den Kontostand ist erledigt:** meiste Schiffe (4.615), aber leichte Jaeger 2.679 gegen Bomber 27 - genau die Monokultur, die der Fix vom 13.08.2026 beseitigt hat (x4,00 Schiffszahl bei nur x2,23 Flottenmacht). **Leiter nicht monoton** (25 -> 1.855, 100 -> 1.780), weil grosse Bloecke die "geringster Bestand zuerst"-Sortierung sprunghaft verschieben - eine Spanne darueber waere sinnlos. **Offene Diskrepanz:** Simulation 1.153 Kampfschiffe nach 7 Tagen gegen real 178 nach Wochen, bei nachweislich gleichem Takt - vermutlich laufende Verluste (Elite 30 %, Expeditionen 20 %, Piratenbasen alle ~40 min) oder nicht durchgehender Heartbeat; **vor jeder Deckel-Aenderung zu klaeren**. Empfohlene Reihenfolge: Verlustfrage, dann Reserve deckeln, dann erst der Stueckzahldeckel als FESTE Zahl. |
| 28.08.2026 (siebte Session, dritter Teil) | **Die drei offenen Fragen zum Reichen Fund sind entschieden - vom Nutzer delegiert, begruendet eingetragen, umkehrbar. NICHTS GEBAUT.** Protokoll `reicherfund_13_entscheidungen.txt`; Verfahren wie bei f = 12, Befund G, K1b, K4 und v_p016 selbst. **(C) Die Eskorten-Praemie wird ausgeschlossen** - ein Minenfund ist keine Kampfbelohnung, und vor allem ist die Kopplung derselbe Bautyp, der den Fund ueberhaupt erst zum Problem gemacht hat: er driftete, weil eine Aenderung an der MISSIONSDAUER eine Mechanik anderswo mitskalierte, und die Praemien-Kopplung taete dasselbe fuer jede kuenftige Aenderung an der Eskorte. Preis: Ertragsanteil einer Eskorte -69 %, Schutzwirkung unberuehrt; falls Spieler daraufhin Eskorten weglassen, ist die Antwort eine hoehere Praemie, nicht die Rueckkehr zur Kopplung. **(A) Der Faktor bleibt bei 0,875, NICHT gegen den Median kalibriert (0,709).** Grund: der gemessene Unterschied klingt mit der Laufzeit ab - +26,0 % ueber 7 Tage, +14,6 % ueber 14, **+1,7 % im Erwartungswert**; ueber die Zeitraeume, in denen hier gerechnet wird, bekommt ein Spieler unter beiden Formen gleich viel, nur anders verteilt. Eine Median-Kalibrierung waere eine echte Kuerzung des Erwartungswerts um 19 % gewesen und haette den Nenner verkleinert, damit den Anteil von Raid und Elite gehoben und **Entscheidung 3 samt Abnahmekriterium 5 neu kalibrierungsbeduerftig gemacht** - eine Anhebung ist in dieser Richtung ungefaehrlich, eine Kuerzung nicht. Zu beachten bleibt: jede WOCHENkennzahl liest in den ersten Wochen hoeher (Woche-1-Baseline, K5, Kurzlaeufe). **(B) Der Wegfall der Zeitpunkt-Abhaengigkeit ist gewollt** - sie ist die Ursache der Drift; bleibt die Kompoundierung, skaliert die naechste Aenderung an einer Missionsdauer den Fund wieder mit. Das Gluecksspiel bleibt vollstaendig. Auflage: Code-Kommentar und Changelog muessen den Verlust mit Datum und Grund festhalten. **Eigene fruehere Formulierung eingeschraenkt statt stehengelassen:** "widerspricht 12b in seinem Kern" ist fuer die Einzelwoche richtig und fuer die Balance zu scharf. **Einbaupaket steht fest** (0,16 / 0,875 / Bemessung am Mining allein / zeitpunktunabhaengig) - **der Einbau selbst ist NICHT entschieden.** |
| 28.08.2026 (siebte Session, zweiter Teil) | **Der K3b-Vorbehalt aus `reicherfund_11.txt` Abschnitt 13 ist geprueft und quantifiziert. NICHTS AM SPIELCODE GEBAUT.** Protokoll `k3b_vorbehalt.txt`, Rohdaten `k3b_vorbehalt.json` (18 Laeufe), geaendert `sim13_lauf.mjs` (K3c ergaenzt, **K3b unveraendert** - Muster K1/K1b und K3/K3b). **Bestaetigt und beziffert: mit `--mensch_unterschritte` ueberschaetzt K3b um 14,7 Punkte** (67,5 statt 52,8 %), weil `probe()` 30x je Stunde die Flags des Zuges der VORSTUNDE liest und am Stundenende gar nicht mehr laeuft. **Zweiter Fundort, bisher nirgends notiert: das gilt auch OHNE den Schalter, sobald das Profil nicht `aktiv` ist** - bei `gelegenheit` handelt der Mensch alle 12 h, bei `abwesend` an den Tagen 1-14 nie, die Probe liest dann bis zu zwei Wochen alte Flags. Punkt 4 faehrt genau diese Profile. **Dritter und schwerster Befund: K3b hat bei `abwesend` ueber 30 Tage einen Nenner von 21.600 Proben, die auf 17 Zuege zurueckgehen** - formal dreistellig, sachlich leer; dieselbe Fehlerform wie "K5 formal erhoben und sachlich leer". Vorab ausgezaehlt, Zuege in 30 Tagen: aktiv 720, gelegenheit 60, abwesend 17. **Empfehlung fuer Punkt 4: `--mensch_unterschritte` kann verwendet werden (K2 ist nicht betroffen und der Schalter existiert genau dafuer), aber K3c zitieren - fuer `aktiv` tragfaehig, fuer `gelegenheit` mit Vorbehalt, fuer `abwesend` gar nicht.** Eingebaute Gegenprobe: bei `aktiv` ohne Schalter sind K3b und K3c auf die Nachkommastelle identisch (48,2 %), die neue Kennzahl erfindet nichts. Grenzen: 7 Tage, Treiber `economy`, drei Wiederholungen - unter `tick` ist die Verzerrung nicht gemessen. Zwei neue Eintraege in der Fallen-Liste. |
